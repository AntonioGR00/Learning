  ...
  return (
    <div className="space-y-6">
      <div className="space-y-2">
        <p className="text-sm font-semibold uppercase tracking-[0.22em] text-[#1b7f5a]">
          Sesion activa
        </p>
        <h2 className="text-3xl font-semibold tracking-[-0.04em] text-[#15231d]">
          Bienvenido, {session.user.fullName}
        </h2>
        <p className="text-sm leading-7 text-[#55635d]">
          La autenticacion con JWT ya esta conectada. El siguiente paso es usar
          este token para proteger rutas y consumir modulos por rol.
        </p>
      </div>

      <div className="grid gap-3 sm:grid-cols-2">
        <SessionCard label="Rol" value={session.user.role} />
        <SessionCard label="Correo" value={session.user.email} />
        <SessionCard label="Usuario ID" value={String(session.user.id)} />
        <SessionCard label="API" value={apiUrl} mono />
      </div>

      <div className="rounded-[24px] bg-[#15231d] p-4 text-sm text-[#f7efe5]">
        <div className="text-xs uppercase tracking-[0.22em] text-[#d6b59f]">
          Access token
        </div>
        <div className="mt-2 break-all font-mono text-[12px] leading-6">
          {session.accessToken}
        </div>
      </div>
      ...
      <button
        type="button"
        onClick={onLogout}
        className="inline-flex items-center justify-center rounded-2xl border border-[rgba(21,35,29,0.12)] px-5 py-3 text-sm font-semibold text-[#15231d] transition hover:bg-[#f1e7db]"
      >
        Cerrar sesion local
      </button>
    </div>
...
function SessionCard({
  ...
  return (
    <div className="rounded-[20px] border border-[rgba(21,35,29,0.08)] bg-[#fffdf8] p-4">
      <div className="text-xs font-semibold uppercase tracking-[0.2em] text-[#8c6d57]">
        {label}
      </div>
      <div className={`mt-2 text-sm text-[#15231d] ${mono ? "font-mono break-all" : "font-semibold"}`}>
        {value}
      </div>
    </div>
  );
